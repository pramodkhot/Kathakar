    ...
    suspend fun unlockEpisode(userId: String, authorId: String, episode: Episode): Resource<Int> {
        ...
        return try {
            ...
            db.runTransaction { t ->
                ...
                t.set(txnRef, CoinTransaction(txnRef.id, userId, CoinTxnType.EPISODE_UNLOCK,
                    -cost, "Unlocked: ${episode.title}", episode.episodeId, Timestamp.now()))
                t.set(aTxnRef, CoinTransaction(aTxnRef.id, authorId, CoinTxnType.AUTHOR_EARNING,
                    credit, "Earned: ${episode.title}", episode.episodeId, Timestamp.now()))
            }.await()
