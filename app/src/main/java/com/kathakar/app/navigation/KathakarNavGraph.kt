...
sealed class Screen(val route: String) {
    ...
    object StoryDetail    : Screen("story/{storyId}") { fun go(id: String) = "story/$id" }
    object EpisodeReader  : Screen("episode/{episodeId}") { fun go(id: String) = "episode/$id" }
}
    ...
    NavHost(navController = navController,
        ...
        composable(Screen.CreateEpisode.route, listOf(
            ...
            CreateEpisodeScreen(storyId = storyId, chapterNumber = ch, authorId = user.userId,
                onDone = { navController.popBackStack() }, onBack = { navController.popBackStack() })
        }
        ...
        composable(Screen.Profile.route) {
            ...
            ProfileScreen(user = user,
                onSignOut = { authVM.signOut(); navController.navigate(Screen.Login.route) { popUpTo(0) { inclusive = true } } },
                onBuyCoins       = { navController.navigate(Screen.BuyCoins.route) },
        ...
        composable(Screen.BuyCoins.route) { ComingSoonScreen("Buy Coins", "Payments coming soon!", onBack = { navController.popBackStack() }) }
        composable(Screen.Subscribe.route) { ComingSoonScreen("Subscriptions", "Subscription plans coming soon.", onBack = { navController.popBackStack() }) }
        composable(Screen.AiWrite.route) { ComingSoonScreen("AI Writing Assistant", "AI features coming soon.", onBack = { navController.popBackStack() }) }
    }
}
